...
async function searchTMDB(
    ...
    const searchUrl =
        `${TMDB_API_BASE}/search/${type}?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)
        }`;
...
async function renameFile(filePath: string): Promise<void> {
    ...
    let newName = "";
    if (type === "movie") {
        newName = `${mediaInfo.title} (${mediaInfo.year}) ${mediaInfo.director}${extname(filePath)
            }`;
    } else {
...
async function main() {
    ...
    if (fileInfo.isDirectory) {
        for await (
            const entry of walk(resolvedPath, { exts: [".mp4", ".mkv", ".avi"] })
        ) {
            if (entry.isFile) {
